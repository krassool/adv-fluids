...
% Calculates the u and v velcoity at locations (xp, yp) due to a source
% panel of strength q placed between (x(1) y(1)) and (x(2) y(2)).
function [u, v] = source_panel_field(q, x, y, xp, yp)
...
u_d = (q/(2*pi))*(-0.5*log(y0_d.^2 + S^2/4 - x0_d.*S + x0_d.^2) + ...
    0.5*log(y0_d.^2 + S^2/4 + x0_d.*S + x0_d.^2));
v_d = (q/(2*pi))*(atan((S/2 - x0_d)./y0_d) - atan((-S/2 - x0_d)./y0_d));
